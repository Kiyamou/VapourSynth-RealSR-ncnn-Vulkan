...
static void process(const VSFrameRef* src, VSFrameRef* dst, const FilterData* const VS_RESTRICT d, const VSAPI* vsapi) noexcept
{
    if (d->vi->format->colorFamily == cmRGB)
    {
        ...
        d->gpuSemaphore->wait();
        d->realsr->process(srcpR, srcpG, srcpB, dstpR, dstpG, dstpB, src_width, src_height, d->vi->format->numPlanes, src_stirde, dst_stride);
...
static void VS_CC filterCreate(const VSMap* in, VSMap* out, void* userData, VSCore* core, const VSAPI* vsapi)
{
    ...
    try
    {
        ...
        d->realsr = new RealSR(gpuId, tta);
        d->realsr->scale = scale;
        d->realsr->tilesize_x = tilesize_x;
        d->realsr->tilesize_y = tilesize_y;
        d->realsr->prepadding = 10;
        d->realsr->load(paramPath, modelPath);
    }
    catch (const std::string &error)
    {
        ...
        vsapi->setError(out, ("RealSR: " + error).c_str());
        vsapi->freeNode(d->node);
        return;
    }

    vsapi->createFilter(in, out, "RealSR", filterInit, filterGetFrame, filterFree, fmParallel, 0, d.release(), core);
}

VS_EXTERNAL_API(void) VapourSynthPluginInit(VSConfigPlugin configFunc, VSRegisterFunction registerFunc, VSPlugin* plugin)
{
    configFunc("com.vapoursynth.realsrnv", "rsnv", "RealSR ncnn Vulkan plugin", VAPOURSYNTH_API_VERSION, 1, plugin);
